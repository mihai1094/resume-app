...
- AI services live in `lib/ai/` with shared utilities and caching.
- AI routes are under `app/api/ai/` and use centralized auth, validation, and error handling.
- See [`.claude/AI_FEATURES.md`](./.claude/AI_FEATURES.md) for the detailed catalog and flow.
...
This repo configures MCP servers in `.mcp.json` (currently `shadcn`). When adding or updating UI components, prefer the shadcn MCP server for current component guidance. For other libraries, prefer local docs in `docs/` and official upstream documentation.

## Automation Helpers

The following helper commands live in `.claude/commands/` for Claude Code users:
...
Codex CLI does not load these slash commands directly. If you need the same workflows, run the underlying npm scripts or follow the steps described in those command files.
...
- **Claude Config (for Claude Code)**: [`.claude/README.md`](./.claude/README.md)
