...
export function useOptimizeFlow(resumes: SavedResume[]) {
  ...
  useEffect(() => {
    const timeoutRef = analysisTimeoutRef;
    return () => {
  ...
  const handleOptimize = async (isRetry: boolean = false) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        // Show toast notification based on error code
      ...
      if (data.meta?.fromCache) {
        aiLogger.debug("Analysis from cache", {
          action: "analyze-ats",
      ...
      if (
        ...
        aiLogger.error("Analysis error", error, {
          action: "analyze-ats",
          errorCode: analysisErr.code,
        });
      ...
      aiLogger.error("Error analyzing resume", error, {
        action: "analyze-ats",
      });
