...
const {
  ...
  mockAnalyzeATSCompatibility: vi.fn(),
}));
...
vi.mock("@/lib/ai/content-generator", () => ({
  analyzeATSCompatibility: mockAnalyzeATSCompatibility,
}));
...
vi.mock("@/lib/api/sanitization", () => ({
  validateJobDescription: vi.fn((jd: string) => jd),
  validateResumeData: vi.fn(),
}));
...
describe("POST /api/ai/analyze-ats", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    ...
    mockAnalyzeATSCompatibility.mockResolvedValue({
      score: 85,
      keywords: { matched: ["React"], missing: ["AWS"] },
    });
  });

  it("returns 200 with analysis and meta for valid input", async () => {
    const req = makeRequest("/api/ai/analyze-ats", {
