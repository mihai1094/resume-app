...
describe("POST /api/ai/score-resume", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    ...
    mockScoreResume.mockResolvedValue({
      overall: 78,
      categories: [],
    });
  });

  it("returns 200 with score and meta for valid input", async () => {
    const req = makeRequest("/api/ai/score-resume", {
    ...
    expect(body.score).toEqual({ overall: 78, categories: [] });
